...
export function ResetPasswordForm() {
  ...
  function handleSubmit(e) {
    ...
    if (username.length < 6) {
      ...
      //If the username is valid, set the error message to false
      setErrorMessageExistsUser(false);
      errorMessageExistsUserScope = false;
    }
    if (username.includes("@") === true) {
      //If the username contains an @ symbol
      emailExists = true;
    } else {
      emailExists = false;
    }
    ...
    if (password === PasswordRepeat) {
      ...
      //If the passwords do not match, set the error message
  ...
  function handleChangeUser(e) {
    ...
    if (username.length < 6) {
      // If the username is less than 6 characters, set the error message
      setErrorMessageUser("Username must be at least 6 characters long");
      setErrorMessageExistsUser(true);
    } else {
      //If the username is valid, set the error message to false
